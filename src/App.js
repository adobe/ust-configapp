...
import './App.css';
import { Alert, ButtonGroup, Button, Collapse, Navbar, NavbarToggler, NavbarBrand, Nav, NavItem, NavLink, Container, Row, Col } from 'reactstrap';
...
const g_remoteurl = "http://localhost:14242/api/v1/";
...
export default class extends Component {
  ...
  onNext = (e) =>{
    ...
    if(s.selected == s.wizsteps.length-1){
      ...
      if(s.selected === 1){
        ...
        if(ldap.host && ldap.username && ldap.password && ldap.base_dn 
          ...
          <Row>
            <Col tag="main" sm={9} className="ml-sm-auto pt-3" role="main">
              <Container fluid={true}>              
                  ...
                  <Row>
                      <Col sm={12} style={{overflowX:"auto", overflowX: "auto", borderRight:"1px solid lightgray", marginBottom: 40, height:320}}>
                          <Alert 
  ...
  processConfigDataForServer(cd){
    /*
    const aufk = Object.keys(g_allUsersFilterOptions).filter(k => k === ldp.ustapp_all_users_filter)[0];
    if(aufk){
      ldp["all_users_filter"] = g_allUsersFilterOptions[aufk];
    }
    cd.directory_users.connectors.ldap_data = ldp;
    */
  ...
  loadConfigFile(configfile){ 
    ...
    .catch(error => alert("Failed to load configuration.\r\nPlease check python service is running."));
  }

  saveConfigFile(configfile, d, connector, callback){
    fetch(g_remoteurl + "config?configfile="+ encodeURIComponent(configfile) + "&connector="+ connector, { 
      method: 'POST',
      headers: {
